...
export default function DescriptionBox({ name, label, text, big, max }: DescriptionBoxProps) {
	const size = `w-full text-xs ${big ? "min-h-72" : max ? "text-xs w-full min-h-160" : "min-h-24"}`;

	return (
		<div className="mt-3 border-solid border-2 p-3 overflow-hidden relative w-full border-black bg-white">
